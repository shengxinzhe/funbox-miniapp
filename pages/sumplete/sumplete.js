...
function generatePuzzle(size, maxNum) {
  ...
  // Ensure at least 1 kept and 1 deleted per row/col for interesting puzzles
  const solution = []
  for (let r = 0; r < size; r++) {
    const row = []
    for (let c = 0; c < size; c++) {
  ...
  for (let r = 0; r < size; r++) {
    const hasKept = solution[r].some(v => v)
    const hasDeleted = solution[r].some(v => !v)
    if (!hasKept) solution[r][Math.floor(Math.random() * size)] = true
    if (!hasDeleted) {
      let idx
  ...
  for (let c = 0; c < size; c++) {
    const colVals = solution.map(row => row[c])
    const hasKept = colVals.some(v => v)
    const hasDeleted = colVals.some(v => !v)
    if (!hasKept) solution[Math.floor(Math.random() * size)][c] = true
    if (!hasDeleted) {
      let idx
